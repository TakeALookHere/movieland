...
class UserSecurityServiceFTest {
    ...
    void testPutUserIntoCache(User user) {
        ...
        UserPrincipal userFromCache = userSecurityService.getFromCache('8495da92-093c-4e5d-b178-82da08b66d7b').get()
        assertEquals(userFromCache.user.id, user.id)
        assertEquals(userFromCache.user.nickname, user.nickname)
        assertEquals(userFromCache.user.email, user.email)
    }
